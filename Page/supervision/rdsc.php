...
select	               A.STORE_CODE TIENDA,
                       ...
                       st.udf1_string cod_super,
                       st.udf2_string supervisor,
                     ...
                     --   and t1.CREATED_DATETIME between to_date('2024-05-26 00:00:00', 'YYYY-MM-DD HH24:MI:SS') ANd to_date('2024-06-01 23:59:59', 'YYYY-MM-DD HH24:MI:SS')
						...
						group by t1.store_code,  t1.employee1_login_name, t1.employee1_full_name, trunc(t1.created_datetime), T1.DOC_NO, t1.receipt_type, t1.disc_amt, A.TIPO, A.META, A.HORA, E.FECHA_INGRESO,st.udf1_string,st.udf2_string
							)A 
							GROUP BY A.STORE_CODE, A.COD_VENDEDOR, A.VENDEDOR, PUESTO, META, CONTRATACION, HORA, A.COD_SUPER,
                       A.SUPERVISOR
							order by 1,2,3)TA
                            GROUP BY COD_SUPER,SUPERVISOR
                            order by COD_SUPER";
  ...
  ?>
      <h3 class="text-center font-weight-bold text-primary">Cadena: <?php echo "Calzado Roy Guatemala"; ?><br><small class="h4 text-primary font-weight-bold text-center"><?php echo "| Año: " . substr($semana, 0, 4) . " | Semana: " . substr($semana, -2) . " | Meta Semana: Q " . number_format(MTSC($tienda, substr($semana, -2), substr($semana, 0, 4), $sbs)[0], 2) . " |" ?></small></br></h3>
    
      <table style="font-size:14px;" class="table table-hover table-sm tbrdsc">
        ...
        <tbody class="align-middle font-size" style="width:100%">
          <?php
          foreach ($resultado as $rdsc) {
          ?>
            <tr>
              <td><?php echo $cnt++ ?></td>    
              <td><b><?php echo $rdsc[0] ?><b></td>
              <td><b><?php echo $rdsc[1] ?><b></td>    
              <td><?php echo ucwords(strtolower($rdsc[2])) ?></td>              
              <td style="<?php echo v_vrs_m($rdsc[3]) ?>"><?php echo iva($iva, $rdsc[3], $sbs) ?></td>
              <td style="<?php echo v_vrs_m($rdsc[4]) ?>"><?php echo iva($iva, $rdsc[4], $sbs) ?></td>
              <td style="<?php echo v_vrs_m($rdsc[5]) ?>"><?php echo iva($iva, $rdsc[5], $sbs) ?></td>
              
              <td><?php echo $rdsc[6] ?></td>
              <td><?php echo $rdsc[7] ?></td>
              <td><?php echo $rdsc[8] ?></td>
              <td><?php echo $sim[0] . " " . number_format($rdsc[4] / $rdsc[7] , 2) ?></td>
              <td><?php echo $rdsc[10] ?></td>
              <td><?php echo $sim[0] . " " . number_format($rdsc[11], 2) ?></td>
              <td><?php echo $sim[0] . " " . number_format($rdsc[12], 2) ?></td>
              <td><?php echo Porcentaje($rdsc[4], $rdsc[3]) . " %" ?></td>
              <td>
                <span class="<?php echo status(Porcentaje($rdsc[4], $rdsc[3])) ?>" style="<?php echo color(Porcentaje($rdsc[4], $rdsc[3])) ?>">
                </span>
              </td>
            </tr>
          <?php

            if ($rdsc[3] === 'VACACIONISTA') {
              $rdsc[4] = 0;
            }
